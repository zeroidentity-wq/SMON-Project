...
log() {
    ...
    local message="$2"
    local timestamp_iso
    # Prefer ISO8601 in UTC with milliseconds if available
    if date '+%Y' >/dev/null 2>&1 && date -u '+%Y-%m-%dT%H:%M:%S.%3NZ' >/dev/null 2>&1; then
    ...
    # Build JSON or text based on LOG_FORMAT
    if [ "${LOG_FORMAT}" = "json" ]; then
        # Basic JSON escaping for message
        ...
        local pid="$$"
        local json_line
        json_line="{\"timestamp\":\"${timestamp_iso}\",\"level\":\"${level,,}\",\"service\":\"${SERVICE_NAME}\",\"host\":\"${HOST_NAME}\",\"pid\":${pid},\"message\":\"${esc_message}\"}"

        # Console pretty print if enabled and jq exists; file keeps one-line JSON
        if [ "${JSON_PRETTY}" = "true" ] && command -v jq >/dev/null 2>&1; then
            echo "$json_line" | jq .
            echo "$json_line" >> "$LOG_FILE"
        else
            echo "$json_line" | tee -a "$LOG_FILE"
        fi

        # Send JSON to syslog for error/critical
        if [[ "$level" == "ERROR" || "$level" == "CRITICAL" ]]; then
            logger -p daemon.err "$json_line"
        fi
    else
        # Fallback text format: human-friendly
        local ts_human=$(date '+%Y-%m-%d %H:%M:%S')
        echo "$ts_human  [$level] - $message" | tee -a "$LOG_FILE"
        if [[ "$level" == "ERROR" || "$level" == "CRITICAL" ]]; then
            logger -p daemon.err "$ts_human [$level] $message"
        fi
    fi
}
...
EOF
    ...
    mysql --defaults-file="$temp_mysql_config" --connect-timeout=5 --quick --reconnect=FALSE -N <<EOF
...
restart_process() {
    ...
    while [ $attempt -le "$max_attempts" ]; do
        ...
        case "$strategy" in
            "custom")
                # First, try enhanced custom execution (block/script/command)
                if execute_enhanced_restart "$process_name" "$system_name"; then
                    : # already executed by enhanced path
                else
                ...
                ;;
            "service")
                # Check enhanced custom execution first (if configured)
                if execute_enhanced_restart "$process_name" "$system_name"; then
                    :
                else
                    # fallback to initctl if no custom command
...
#
# Enhanced custom restart executor
# Supports executing a full Bash code block (base64-encoded), a script path, or a single command.
# Precedence: restart_block_b64 > restart_script_path > restart_command.
# Returns 0 if something was executed (regardless of success of underlying command),
# and logs success/failure accordingly. Returns 1 if nothing configured (caller should fallback).
#
execute_enhanced_restart() {
    local process_name="$1"
    local system_name="$2"

    local restart_block_b64=$(get_process_config "$process_name" "restart_block_b64" "")
    local restart_script_path=$(get_process_config "$process_name" "restart_script_path" "")
    local restart_command=$(get_process_config "$process_name" "restart_command" "")

    # Export context so scripts or blocks can use them
    export PROCESS_NAME="$process_name"
    export SYSTEM_NAME="$system_name"

    if [ -n "$restart_block_b64" ]; then
        if ! command -v base64 >/dev/null 2>&1; then
            log "ERROR" "restart_block_b64 configured for $process_name but 'base64' is not available"
            return 0
        fi
        local tmp_script
        tmp_script=$(mktemp)
        # Decode block to temp script
        echo "$restart_block_b64" | base64 -d > "$tmp_script" 2>/dev/null
        chmod +x "$tmp_script" 2>/dev/null
        log "INFO" "Executing restart code block for $process_name (decoded to $tmp_script)"
        # Execute with bash; allow block to use PROCESS_NAME and SYSTEM_NAME
        if bash "$tmp_script"; then
            log "INFO" "Restart code block completed for: $process_name"
        else
            log "ERROR" "Restart code block failed for: $process_name"
        fi
        rm -f "$tmp_script" 2>/dev/null
        return 0
    fi

    if [ -n "$restart_script_path" ]; then
        # Allow placeholder substitution in the path/args
        local script_cmd=$(echo "$restart_script_path" | sed "s/%s/$system_name/g")
        log "INFO" "Executing restart script for $process_name: $script_cmd"
        if eval "$script_cmd"; then
            log "INFO" "Restart script executed for: $process_name"
        else
            log "ERROR" "Restart script failed for: $process_name"
        fi
        return 0
    fi

    if [ -n "$restart_command" ]; then
        # Backward compatible path: single-line command, supports %s substitution
        local cmd=$(echo "$restart_command" | sed "s/%s/$system_name/g")
        log "INFO" "Executing custom restart command for $process_name: $cmd"
        if eval "$cmd"; then
