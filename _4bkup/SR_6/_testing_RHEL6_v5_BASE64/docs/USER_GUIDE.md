...
| restart_script_path    | Cale script sau comandă cu argumente de rulat la restart                  | Oricând; are prioritate peste `restart_command` | restart_script_path = /usr/local/bin/restart_myapp.sh %s |
| restart_block_b64      | Bloc Bash multi-linie, Base64, rulat ca script temporar                   | Oricând; cea mai mare prioritate                | vezi exemplul de mai jos                   |
...
#### 5. Restart cu script path (prioritar față de `restart_command`)
```ini
[process.api]
restart_strategy = service
system_name = my-api
restart_script_path = /usr/local/bin/rolling_restart.sh %s --grace 10
health_check_command = curl -fsS http://localhost:8080/health > /dev/null
```

#### 6. Restart cu bloc Bash multi-linie (Base64) – poate include logică complexă
Pași:
- Scrieți un script Bash multi-linie.
- Encodați conținutul cu `base64 -w0` și plasați rezultatul în `restart_block_b64`.

Exemplu script (ne-encodat) pentru referință:
```bash
#!/usr/bin/env bash
set -euo pipefail
echo "Stopping $SYSTEM_NAME..."
systemctl stop "$SYSTEM_NAME" || true
echo "Applying maintenance..."
if grep -q critical /var/log/app/errors.log; then
  echo "Critical errors detected, aborting restart" >&2
  exit 1
fi
...
Encodare într-o singură linie:
```bash
base64 -w0 ./script.sh
```

Config ini cu blocul rezultat:
```ini
[process.myapi]
restart_strategy = custom
system_name = my-api
restart_block_b64 = <LIPITI_AICI_B64>
health_check_command = curl -fsS http://localhost:8080/health > /dev/null
health_check_timeout = 10
```

Fără script extern (totul direct în config.ini):
- Nu este obligatoriu să păstrați un fișier `.sh`. Puteți genera Base64 direct dintr-un heredoc și lipi rezultatul în `config.ini`.

```bash
# Generează Base64 fără a crea un fișier pe disc
base64 -w0 <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
systemctl stop "$SYSTEM_NAME" || true
# toată logica de restart aici (if/while/etc.)
systemctl start "$SYSTEM_NAME"
EOF
```

Note operaționale:
- La runtime, blocul este decodat într-un fișier temporar executabil și apoi șters.
- Variabile disponibile în bloc/script: `PROCESS_NAME`, `SYSTEM_NAME`.
- Precedență execuție: `restart_block_b64` > `restart_script_path` > `restart_command` > fallback (`initctl`/`pgrep`).
- Necesită utilitarul `base64` pe host pentru a folosi `restart_block_b64`.

Note:
- Precedență de execuție: `restart_block_b64` > `restart_script_path` > `restart_command` > fallback (`initctl`/`pgrep` etc.).
- Variabilele de context disponibile în bloc/script: `PROCESS_NAME`, `SYSTEM_NAME`.
- Necesită utilitarul `base64` pentru `restart_block_b64`.
