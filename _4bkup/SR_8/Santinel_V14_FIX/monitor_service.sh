...
#               - Pentru "service": `systemctl start/restart` cu `system_name`
#               - Pentru "custom": `systemctl start/restart` cu `system_name` (fallback)
    ...
    while [ $attempt -le "$max_attempts" ]; do
        ...
        case "$strategy" in
            ...
            "process")
                ...
                ;;
            *)
                log "ERROR" "RESTART LOG: Unknown restart strategy for $process_name: $strategy"
                ;;
        esac
