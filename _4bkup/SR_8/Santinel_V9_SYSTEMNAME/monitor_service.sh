    ...
    while [ $attempt -le "$max_attempts" ]; do
        ...
        case "$strategy" in
            "service")
                ...
                if systemctl restart "$system_name" 2>/dev/null; then
                    log "INFO" "RESTART LOG: Service restart command executed for: $system_name"
                    # Perform health check after restart
                    if perform_health_check "$process_name"; then
                        log "INFO" "RESTART LOG: Successfully restarted and verified service: $system_name"
                        return 0
                    else
                        log "ERROR" "RESTART LOG: Service restart command succeeded but health check failed for: $system_name"
                    fi
                fi
                ;;
            ...
            "process")
                ...
                if pkill "$system_name"; then
                    log "INFO" "RESTART LOG: Successfully killed process: $system_name"
                    sleep "$restart_delay"
                    if "$system_name" >/dev/null 2>&1 & then
                        log "INFO" "RESTART LOG: Process start command executed for: $system_name"
                        # Perform health check after restart
                        if perform_health_check "$process_name"; then
                            log "INFO" "RESTART LOG: Successfully started and verified process: $system_name"
                            return 0
                        else
                            log "ERROR" "RESTART LOG: Process start command succeeded but health check failed for: $system_name"
                        fi
                    fi
                fi
                ;;
            ...
            "custom")
                ...
                # Replace %s with system_name if present in the command
                restart_command=$(echo "$restart_command" | sed "s/%s/$system_name/g")
                ...
                if eval "$restart_command" >/dev/null 2>&1; then
                    log "INFO" "RESTART LOG: Custom restart command executed for: $system_name"
                    # Perform health check after restart
                    if perform_health_check "$process_name"; then
                        log "INFO" "RESTART LOG: Successfully restarted and verified process: $system_name"
                        return 0
                    else
                        log "ERROR" "RESTART LOG: Custom restart command succeeded but health check failed for: $system_name"
                    fi
                else
                    log "ERROR" "RESTART LOG: Custom restart command failed for: $system_name"
                fi
                ;;
