...
# Oprește toate serviciile în stare de alarmă
stop_alarm_services() {
    log "INFO" "Se opresc serviciile în stare de alarmă..."
    
    # Obține lista serviciilor în alarmă
    local services=$(mysql -N -u"$DB_USER" -p"$DB_PASS" -h"$DB_HOST" "$DB_NAME" <<EOF
...
EOF
    ...
    if [ -z "$services" ]; then
        log "INFO" "Nu există servicii în stare de alarmă"
        return
    fi
    
    # Oprește fiecare serviciu
    echo "$services" | while read service; do
        log "INFO" "Se oprește serviciul: $service"
        if systemctl stop "$service"; then
            log "INFO" "Serviciul $service a fost oprit cu succes"
        else
            log "ERROR" "Nu s-a putut opri serviciul $service"
        fi
    done
}
...
main() {
    ...
    while true; do
        ...
        echo "2. Setează un serviciu specific în alarmă şi bifează"
        echo "3. Setează toate serviciile în alarmă"
        echo "4. Resetează toate alarmele"
        echo "5. Oprește serviciile în alarmă"
        echo "6. Ieșire"
        echo
